...
// Picture upload timer
#include <Ticker.h>
Ticker timer;

// Picture upload server
const char *upload_host = "194.44.63.37";
static IPAddress upload_ip(194, 44, 63, 37);
// static IPAddress upload_ip(192, 168, 1, 9);
const int upload_port = 8999;
// const int upload_port = 80;
const char *upload_path = "/upload/action";
const int upload_interval = 30; // in seconds
...
/*
const char *ssid = "PVL-IOT"; // Put your SSID here
const char *password = "lv1v0it2017"; // Put your PASSWORD here
*/
const char *ssid = "R_M6"; // Put your SSID here
const char *password = "mazda6roman"; // Put your PASSWORD here

static IPAddress ip(192, 168, 3, 103); // static IP used for browser access: http://IPaddress
...
/////////////////////////////////////////////
// capture still image and push to server  //
/////////////////////////////////////////////
void camCaptureAndPush(ArduCAM myCAM) {
  ...
  client.connect(upload_ip, upload_port);

  int max_retry = 50; int counter = 0;
  Serial.print("Connecting to server " + String(upload_host) + ":" + String(upload_port) + " ");
  while(!client.connected() && counter < max_retry) {
    delay(1000);
    Serial.print(".");
    counter++;
  }
  ...
  if (!client.connected()) {
    Serial.println("Unable to connect to server " + String(upload_host) + ":" + String(upload_port));
    return;
  }

  Serial.println("Client status: " + String(client.status()));
  ...
  client.println("Host: " + String(upload_host) + ":" + String(upload_port));
  ...
  Serial.println("Host: " + String(upload_host) + ":" + String(upload_port));
  ...
  while (len) {
    ...
    if (!client.connected()) {
      Serial.println("Lost connection while uploading image!");
      break;
    }
...
void handleNotFound() {
  ...
  message += "<h1>I-Met Project</h1>\n";
  message += "<p>IoT Program @ Lviv Politechnik</p>\n";
  if (errMsg != "")
  ...
  else
  {
    ...
    message += " &nbsp;  <a style=\"font-size:12px; font-weight:bold;\" href=\"http://" + ipStr + "/push\">Push to Server</a>";
  ...
  message += "</form> \n";
  message += "<p>Prepared by Fantastic-Four Team @ Lviv IoT 2017</p> \n";
  message += "</body></html>\n";
...
/////////////////////////////////////////
void pushImage() {
  Serial.println("Pushing the taken image to server " + String(upload_host));

  camCaptureAndPush(myCAM);
...
void setup() {
  ...
  server.on("/push", pushImage);
  server.onNotFound(handleNotFound);
  server.begin();
  Serial.println("Server started");

  // setup picture upload handler
  // timer.attach(upload_interval, pushImage);
